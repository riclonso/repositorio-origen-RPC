...
// Dos tipos de regla: de un conjunto de columnas, al menos una debe traer valor (si todas vienen
// vacías, se rechaza); o una columna de fecha debe caer dentro del rango de la ventana de carga
// vigente para esa subida (RF-15). Agregar un tipo nuevo es una decisión de producto que exige
// código nuevo (el evaluador que las ejecuta contra un archivo real), así que vive en este
// arreglo fijo, mismo criterio que `TIPOS_DATO_COLUMNA`.
export const TIPOS_REGLA_VALIDACION = ["ALGUNA_COLUMNA_CON_VALOR", "FECHA_DENTRO_DE_VENTANA_VIGENTE"] as const;
