...
| `modules/usuarios/`    | Implementado | Mantenedor de usuarios (RF-06/RF-13/RF-16): listar, crear sin contraseña, editar, activar/desactivar y definir la contraseña por dos vías: enviar/reenviar un enlace (`POST .../enlace-contrasena`) o fijarla manualmente el admin (`PUT .../contrasena`). Los endpoints tienen guard propio. Ver [docs/arquitectura.md](arquitectura.md#alta-sin-contraseña-y-activación-por-enlace-rf-13). |
| `modules/tipoEstablecimiento/` | Implementado | Mantenedor de tipos de establecimiento (RF-14): catálogo chico (listado simple) con `nombre`, unicidad por `nombreNormalizado` derivado (`shared/utils/texto.ts`), crear/editar/activar/desactivar. Endpoints bajo `app/api/tipos-establecimiento/` con guard propio. |
| `modules/establecimiento/` | Implementado | Mantenedor de establecimientos (RF-14): `rut` (único, dígito verificador, editable), `nombre`, `direccion` (obligatoria) y `tipo` por FK. Listado con búsqueda `unaccent` + paginación en servidor. Crear/editar/activar/desactivar. Endpoints bajo `app/api/establecimientos/` con guard propio. Sin auditoría por ahora. |
